...
value class Location(val packedCoordinates: Int) {
    ...
    val plane: Int get() = (packedCoordinates shr 28) and 0x3
    ...
    fun transform(xOffset: Int, yOffset: Int, levelOffset: Int = 0) = Location(
        x = x + xOffset,
        z = z + yOffset,
        plane = plane + levelOffset
    )

    override fun toString(): String =
        "Location(packedCoordinates=$packedCoordinates, x=$x, z=$z, plane=$plane, zoneX=$zoneX, zoneZ=$zoneZ, zoneId=$zoneId, regionX=$regionX, regionZ=$regionZ, regionId=$regionId)"
}

fun Location.withinDistance(other: Player, distance: Int = 14): Boolean {
    if (other.location.plane != plane) return false
